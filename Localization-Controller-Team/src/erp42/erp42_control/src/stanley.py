...
class Stanley(object):
    def __init__(self):
        ...
        self.__k = rospy.get_param(
            "/stanley_controller/c_gain", 0.8)  # control gain
        self.__hdr_ratio = rospy.get_param(
            "/stanley_controller/hdr_ratio", 0.03)
        
        

        # self.__k = rospy.get_param(
        #     "/stanley_controller/c_gain", 0.4)  # control gain
        # self.__hdr_ratio = rospy.get_param(
        #     "/stanley_controller/hdr_ratio", 0.8)
    ...
    def getCGain(self):    #get -> get함수는 클래스 바깥에서 어떤 인자의 게인값을 가지고 오고 싶을때 게인을 가져와서 리턴해주는 친구에요
        return self.__k

    def setCGain(self, value):  #set -> gain을 설정해줌 (여기서는 C GAIN)
        self.__k = value
        print("SET C GAIN")
        return self.__k
