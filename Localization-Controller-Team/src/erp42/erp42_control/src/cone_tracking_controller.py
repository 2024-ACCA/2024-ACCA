...
class Drive:
    ...
    def publish_cmd(self):
        ...
        if path_type == "global":
            ...
            if last_target_idx == (len(str(self.path))-1): #수정 str
                last_target_idx = 0
        else:
            last_target_idx = 0

        steer, target_idx, hdr, ctr = self.st.stanley_control(self.state, self.path.cx, self.path.cy, self.path.cyaw, last_target_idx, reverse=False)
        ...
        adaptedspeed = self.ss.adaptSpeed(path_speed,hdr,ctr,min_value=3,max_value=20)
...
def main():
    ...
    path_tracking = PathHandler("path/tracking")
    ...
    while not rospy.is_shutdown():
        try:
            d.publish_cmd()
            print(d.last_target_idx_global)
